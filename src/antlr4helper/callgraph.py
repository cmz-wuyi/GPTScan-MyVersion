...
modifier_whitelist = json.load(open("modifier_whitelist.json"))
...
class CallGraph:
        ...
        for root, dirs, files in os.walk(self.root):
            for file in files:
                if "node_modules" in file or "node_modules" in root:
                    continue
                skip_flag = False
                for folder in os.path.split(root):
                    if "test" == folder.lower() or "tests" == folder.lower() or "testing" == folder.lower() or "unittest" == folder.lower() or "unit_test" == folder.lower() or "unit tests" == folder.lower() or "unit_testing" == folder.lower():
                        skip_flag = True
                        break
                if skip_flag:
                    continue
    ...
    def __run_jar(self):
        subprocess.run(["java", "-jar", "jars/SolidityCallgraph-1.0-SNAPSHOT-standalone.jar", self.root, "callgraph.json"], stdout=subprocess.DEVNULL)
